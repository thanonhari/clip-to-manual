...
## 6. 🧠 พจนานุกรมศัพท์ AI Engineering & Prompt Concepts สำหรับผู้เริ่มต้น

| คำศัพท์เชิงวิศวกรรม AI | ความหมายเข้าใจง่าย | ตัวอย่างการทำงานจริง |
|---|---|---|
| **Accordion UI (แอคคอร์เดียน)** | แถบรายการแบบพับเก็บและคลิกกางออกได้ | เหมือนเครื่องดนตรีหีบเพลง (Accordion) ที่ยืดหดได้ เช่น แถบคอร์ส KruBank แสดง 1 บรรทัด พอกดคลิกจะกาง 10 ตอนย่อยออกมา |
| **Parent-Child Hierarchy (แม่-ลูก)** | โครงสร้างความสัมพันธ์แบบลำดับชั้น | ชุดคอร์สแม่ (Course Series) บรรจุตอนลูก (Sub-Episodes EP.1 - EP.10) เอาไว้ข้างใน |
| **Master-Detail View** | การแสดงผลแบบสรุปภาพรวมคู่กับรายละเอียดเจาะลึก | รายการแถวแสดงภาพรวม Master Bundle เมื่อคลิกจะเปิดเจาะลึก (Drilldown) รายละเอียดย่อย |
| **Truncation & Fixed Grid Alignment** | การตัดทอนตัวหนังสือและตรึงความกว้างคอลัมน์ | กำหนดให้หัวตารางและแถวตรงกันเป๊ะ แม้ชื่อตอนจะสั้นยาวไม่เท่ากัน หน้าจอก็จะไม่โย้หรือเบี้ยว |
| **Modal Dialog (โมดอล)** | หน้าต่างลอยทับหน้าจอหลัก | หน้าต่างสรุปรายละเอียดคลิปที่เด้งขึ้นมากลางจอ พร้อมปุ่มปิดและปุ่มอ่านคู่มือ |
| **Stealth Mode (โหมดพรางตัว)** | การลบลายเซ็นบอทออก เพื่อไม่ให้ถูกตรวจจับ | ปลดสัญญาณ `navigator.webdriver` ทำให้ระบบ TikTok/FB มองว่าเราคือมนุษย์เล่น Chrome บนคอมจริง |
| **Human Handshake (การจับมือยืนยันตัวตน)** | การยืนยันตัวตนด้วยคนจริงผ่านมือถือ | เช่น การสแกน QR Code จากแอปมือถือเข้าคอมพิวเตอร์ เป็นวิธีที่ปลอดภัยที่สุด |
| **Humanized Jitter & Delays** | การสุ่มหน่วงเวลาและพฤติกรรมเลียนแบบคน | สุ่มรอ 3-7 วินาทีและเลียนแบบการขยับเมาส์ เพื่อป้องกันระบบบอทตรวจจับความผิดปกติ |
| **Rate Limiting (การควบคุมอัตราความถี่)** | การจำกัดจำนวนครั้งในการยิงคำสั่งในแต่ละช่วงเวลา | เช่น จำกัดให้โพสต์วันละ 3-5 คลิป ห่างกัน 1-2 ชั่วโมง ไม่โพสต์รัวๆ จนโดนบล็อก |
| **Embedded Database (ฐานข้อมูลในตัว)** | ฐานข้อมูลแบบไฟล์เดี่ยว รันเร็ว ไม่กินแรม | เช่น SQLite (ไฟล์ `.db` เดียว) เก็บประวัติการโพสต์และโปรไฟล์ในเครื่องลุงเพชร ไม่ต้องต่อเน็ต |
